...
### select = 2 : 投票情報のみ
    ...
    for i in range(5):
        ...
        if quest_result[i] == 0:
            ...
            quest_info[i] = f"{quest_info[i]}：成功"
        if select == 1 and quest_member[i] != None:
                        ...
                        for i in range(game_member_num):
                            if ctx.author.id == avalon_user[i][2]:
                                ...
                                if avalon_quest[i] < 2:
                                    ...
                                    embed = discord.Embed(title=f"{avalon_user[i][1]}",description="投票を受け付けました")
                                    await msgch.send(embed=embed)
                                else:
                                    sql = f"update `avalon_quest` \
                                    ...
                                    if command_accept == 2:
                                        ...
                                        await msgch.send(f"{avalon_user[i][1]}が上書きしました。")
                                    else:
                            ...
                            if ctx.author.id == avalon_user[num][2]:
                                ...
                                if avalon_user[num][3] == 15:
                                    ...
                                    if quest_cnt%2 == 1 and command_accept == 8:
                                        command_accept = 16
                                        await msg.send(f"あなたは青陣営のため、強制的に成功へ変更しました。")
                                    elif(quest_cnt == 2 or quest_cnt == 4) and command_accept == 16:
                                        command_accept = 8
                                    ...
                                    if quest_success_cnt == 3:
                                        ...
                                        if game_otome == 1 and (quest_cnt >= 2 and quest_cnt <= 4):
                                            ...
                                            if fail_num >= 3:
                                                ...
                                                db.execute(sql, value)
                        if check == 0:
                            await ctx.channel.send(f"あなたはクエストに参加していません。：{comment}")
                    else:
                        await ctx.channel.send(f"このコマンドは無効です。：{comment}")
                ...
                # kill : 暗殺
                    ...
                    if ctx.author.id == avalon_user[kill_member][2]:
                        ...
                        if len(select_member_match) == 1:
                            ...
                            if select_member_num >= 0 and select_member_num <= game_member_num-1:
                                sql = "配役は以下の通りです。"
                                for i in range(game_member_num):
                                    sql = f"{sql}\n{i+1} : {avalon_user[i][1]} : {avalon_role[avalon_user[i][3]][1]}"
                            ...
                            if role_find(game_member_num, avalon_user, 31) != None:
                                ...
                                for i in range(2):
                                    ...
                                    if select_member_num[i] >= 0 and select_member_num[i] <= game_member_num-1:
                                        continue
                                    else:
                                        err_flg = 1
                            if len(select_member_match) != len(set(select_member_match)):
                        ...
                        await ctx.channel.send(f"あなたは暗殺者ではありません。")
                else:
                        await ctx.channel.send(f"このコマンドは無効です。：{comment}")
            else:
                await ctx.channel.send(f"このコマンドは無効です。：{comment}")
