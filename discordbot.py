...
async def on_message(ctx):
    ...
    if game_status == 2:
        for i in range(game_member_num):
            if game_member[i] != None:
                if i == 0:
                    sql = f"{sql}\ngame_member = {game_member[i]}"
                else:
                    sql = f"{sql}, {game_member[i]}"

        if game_member[0] != None:
            print(sql)
            print(len(game_member))
            ...
            if game_status == 1:
                if comment[0:3] == 'ds ':
                    ...
                    try :
                        ...
                        # print(game_role)
                        if game_role > 0 and len(avalon_role_auto[game_member_num]) :
            ...
            if comment[0:2] == 's ' or comment[0:7] == 'select ' or comment[0:3] == '選択 ':
                if game_phase == 0:
                    ...
                    if len(select_member_match) != quest_member_num[game_member_num][quest_cnt-1][0]:
                        ...
                        i = 1
                        for i in range(game_member_num) :
                            sql = f"update `avalon_data` set `game_member{i+1}` = \
                            NULL where id = 0"
                            db.execute(sql)
                            i = i + 1
