...
async def hello(ctx):
    ...
    await bot.send(f"どうも、{ctx.message.author.name}さん！")
    await bot.send(f"どうも、{ctx.message.author}さん！")
    await bot.send(f"どうも、{ctx.message.author.name}さん！", id = ctx.message.author.id)
