...
async def on_ready():
    ...
    db.execute(sql, (0,0,1,0,0,0,0,1,0))
    # テーブル作成 ユーザ情報
...
async def on_message(ctx):
    ...
    for i in rows:
        ...
        game_stop=i[7]
        game_member_num=i[8]
        if game_status == 2 and game_phase == 1:
    ...
    if game_status == 0:
            ...
            if comment[0:3] == 'ds ':
                ...
                try :
                    ...
                    if game_role > 0 and game_role <= len(avalon_role_auto[game_member_num]) :
                        ...
                        await ctx.channel.send(f"デッキを1に設定しました。")
                    db.execute(sql)
                except :
                    await ctx.channel.send(f"現在このコマンドは無効です。：{comment}")
            ...
            if game_member_num > 4:
                ...
                for i in range(game_member_num):
                    if i == 0:
                        ...
                        sql = f"{sql}\n□{i+1}:{ary[i][0]}"
                embed = discord.Embed(title="ゲームメンバー",description=f"{sql}")
                await ctx.channel.send(embed=embed)
                await ctx.channel.send('第１クエスト：1回目の選出です。')
        ...
        if comment == 'stop' or comment == '停止':
            ...
            if comment[0:2] == 's ' or comment[0:7] == 'select ' or comment[0:3] == '選択 ':
                if ctx.author.name == avalon_user[select_member][1]:
                    if game_phase == 0:
                        if game_member_num == 5:
            ...
            for i in range(game_member_num):
                if ctx.author.name == avalon_user[i][1]:
                        ...
                        for k in range(game_member_num):
                            if k == 0:
                                if avalon_quest[k] > 1 or i == k:
                                    vote_msg = f"投票完了:{avalon_user[k][1]}"
                                else:
                                    vote_msg = f"投票未完:{avalon_user[k][1]}"
                            else:
                                if avalon_quest[k] > 1 or i == k:
                                    vote_msg = f"{vote_msg}\n投票完了:{avalon_user[k][1]}"
                                else:
                                    vote_msg = f"{vote_msg}\n投票未完:{avalon_user[k][1]}"
                        if command_accept == 2:
                ...
                if avalon_quest[i] > 1:
                    ...
                    if i == game_member_num-1:
                        if accept_cnt > reject_cnt:
                            ...
                            `game_phase`= {game_phase} where id = 0"
                            db.execute(sql)
